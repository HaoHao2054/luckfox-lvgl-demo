...
namespace Bsp {
    ...
    bool SysfsGpio::pathExists(const std::string &path) { return access(path.c_str(), F_OK) == 0; }
    ...
    SysfsGpio::SysfsGpio(int pinNumber) :
        m_pinNumber(pinNumber), m_gpioPath(GPIO_SYSFS_BASE_PATH + "gpio" + std::to_string(pinNumber) + "/"),
    ...
    bool SysfsGpio::isExported() const { return pathExists(m_gpioPath); }
